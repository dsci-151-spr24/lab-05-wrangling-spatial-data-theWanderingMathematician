...
- *Seeing there are 3 Denny's locations and 2 La Quinta locations in Alaska, we need to calculate 6 distances to determine the distances between each pair.*
...
- *There are `r nrow(dn_lq_ak)` observations in the `dn_lq_ak` dataframe. The names of the variables in this dataframe are `r names(dn_lq_ak)`.*
...
- *We would use the `mutate` function to add new variables to a data frame.*
...
dn_lq_ak_mindist %>%
  summarise(min = min(closest), mean = mean(closest), max = max(closest))
```
- *The minimum distance between a Denny's and a La Quinta in Alaska is `r min(dn_lq_ak_mindist$closest)` kilometers, while the average distance from a Denny's location to the closest La Quinta is `r mean(dn_lq_ak_mindist$closest)`km. The middle value is much closer to the higher value than to the lower one.*
...
```{r}
dn_lq_nc_dist <- dennys %>%
  filter(state == "NC") %>%
  full_join(filter(laquinta, state == "NC"), by = "state") %>%
  mutate(distance = haversine(longitude.x, latitude.x, longitude.y, latitude.y, round = 3))

dn_lq_nc_mindist <- dn_lq_nc_dist %>%
  group_by(address.x) %>%
  summarise(closest = min(distance))
...
- *The minimum distance between a Denny's and a La Quinta in North Carolina is `r min(dn_lq_nc_mindist$closest)`km, while the highest is `r max(dn_lq_nc_mindist$closest)`km. In addition to this, the average distance from a Denny's location to the closest La Quinta is `r mean(dn_lq_nc_mindist$closest)` kilometers.*

10. Repeat the same analysis for Texas.

```{r}
dn_lq_tx_dist <- dennys %>%
  filter(state == "TX") %>%
  full_join(filter(laquinta, state == "TX"), by = "state") %>%
  mutate(distance = haversine(longitude.x, latitude.x, longitude.y, latitude.y, round = 3))

dn_lq_tx_mindist <- dn_lq_tx_dist %>%
  group_by(address.x) %>%
  summarise(closest = min(distance))
...
- *The minimum distance between a Denny's and a La Quinta in Texas is `r min(dn_lq_tx_mindist$closest)` kilometers, while the highest is `r max(dn_lq_tx_mindist$closest)`km. In addition to this, the average distance from a Denny's location to the closest La Quinta is `r mean(dn_lq_tx_mindist$closest)`km.*

11. Repeat the same analysis for a state of your choosing, different than the ones we covered so far.

```{r}
dn_lq_ny_dist <- dennys %>%
  filter(state == "NY") %>%
  full_join(filter(laquinta, state == "NY"), by = "state") %>%
  mutate(distance = haversine(longitude.x, latitude.x, longitude.y, latitude.y, round = 3))

dn_lq_ny_mindist <- dn_lq_ny_dist %>%
  group_by(address.x) %>%
  summarise(closest = min(distance))
...
- *The minimum distance between a Denny's and a La Quinta in New York is `r min(dn_lq_ny_mindist$closest)`kilometers, while the highest is `r max(dn_lq_ny_mindist$closest)`km. In addition to this, the average distance from a Denny's location to the closest La Quinta is `r mean(dn_lq_ny_mindist$closest)`km.*

12. Among the states you examined, where is Mitch Hedberg's joke most likely to hold true?
    Explain your reasoning.

- *Examining statistics, even though the mean is lowest in Alaska, this seems mostly to be due to the low number of locations; it seems like it would be more likely to actually find a Denny's and a La Quinta right next to eachother in Texas, since there are over 55 Denny's locations within 2km of the nearest La Quinta, and the overall distribution's likelihood of being in the range decreases (seemingly exponentially) with distance, while in New York the distribution is more spread-out and randomised, and in North Carolina, it peaks at around 40km separation.*
